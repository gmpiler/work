...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			//�?ータセグメントを覚えておく(シス�?�?コールされたときにアプリケーションの�?ータのアクセスするのに�?�?)
			task->ds_base = (int) q;
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		if (i < 8) {
			...
			// 'm not sure about the reason why these four codes are key to success.
			sprintf(s, "\n[%s/CLUSTNO]:0x%x\n", finfo->name, finfo->clustno);
			if(debugMode == 1) cons_putstr0(cons, s);
			sprintf(s, "\n[DSKIMGADR]:0x%x\n", finfo->clustno * 0x200 + 0x3e00);
			if(debugMode == 1) cons_putstr0(cons, s);
		...
		//int api_fread(char *buf(ebx), int maxsize(ecx), int fhandle(eax));
		char s[100];
		...
		/* api_sprintf(char *fileName(ebx), char *sthToWrite(ecx)) */
		char s[30];
		char *addr;
		addr = (finfo->clustno * 0x200 + 0x3e00) + ADR_DISKIMG;
		sprintf(addr, "%s", (char *) ecx + ds_base);
		sprintf(s, "%s", addr);
	}
	return 0;
}
